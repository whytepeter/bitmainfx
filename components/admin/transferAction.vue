      ...
      <v-card-text class="">
        <v-row class="ma-0">
          <v-col cols="12" class="text-subtitle-1 px-0 font-weight-medium">
            <div>Fund Transfer of {{ data && data.amount }} to {{ data.fullName }}</div>
          </v-col>
...
export default {
  ...
  methods: {
    ...mapActions({ initAlert: 'controller/initAlert', updateTransfer: 'admin/updateTransactions' }),
    update (status) {
      if (this.message === '') {
        this.initAlert({
          is: true,
        ...
        const payload = {
          amount: parseInt(this.data.amount.split(',').join('')),
        ...
        this.updateTransfer(payload)
        this.checkIfDone()
      }
    },
    checkIfDone () {
      setInterval(() => {
        if (!this.loading.transfer) {
          setTimeout(() => {
            this.message = ''
            this.toggle(false, {})
